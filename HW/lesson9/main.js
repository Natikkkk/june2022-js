...
for (const info of simpsons) {
    let div = document.createElement(`div`);
    div.classList.add('member');

    let title = document.createElement(`h2`);
    title.innerText = info.name + " " + info.surname + " " + info.age;
    div.appendChild(title);

    let inf = document.createElement(`p`);
    inf.innerText = info.info;
    div.appendChild(inf);
...
for (const course of coursesArray) {
    ...
    main.appendChild(monthDuration);

    let hourDuration = document.createElement('div');
    hourDuration.innerText = course.hourDuration;
    main.appendChild(hourDuration);
...
for (const {title, monthDuration} of coursesAndDurationArray) {
    let div=document.createElement('div');
    div.innerHTML=`<h2>${title} - ${monthDuration}</h2>`;
    document.body.appendChild(div);
}
...
let p = document.createElement(`p`);
p.innerText = 'when pressed, the text will disappear';
p.id='text';
box.appendChild(p);
...
inputAge.name='age';
inputAge.placeholder = 'Pls,enter your age';

let inputButton=document.createElement('button');
inputButton.innerText='Checking';

form.append(inputAge, inputButton);
